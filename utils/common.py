...
"""Common math and coordinate transformation utilities."""
...
def norm(x, axis=None):
    """Calculate the L2 norm of array `x` along specified `axis` (default: all elements)."""
    return (x * x).sum(axis) ** 0.5


def rms(x, axis=None):
    """Compute the root mean square of array `x` along specified `axis` (default: all elements)."""
    return (x * x).mean(axis) ** 0.5


def uvec(x, axis=1):  # turns each row or col into a unit vector
    """Normalize rows or columns of `x` to unit vectors along `axis`=1 (rows default) or 0; retains original shape."""
    return x / (x * x).sum(axis, keepdims=True) ** 0.5


def addcol0(x):  # append a zero column to right side
    """Append a zero column to the right of array `x`, returning a new array with retained original dtype."""
...
def addcol1(x):  # append a ones column to right side
    """Append a ones column to the right of array `x`, returning a new array with retained original dtype."""
...
def cc2sc(x):
    """Convert cartesian coordinates to spherical, input shape (N, 3) or (3,), output shape matches input."""
    s = np.zeros_like(x)
...
def sc2cc(s):  # spherical to cartesian [range, el, az] to [x, y z]
    """Convert spherical coordinates [range, elevation, azimuth] to cartesian coordinates [x, y, z]."""
    x = np.zeros_like(s)
...
def pixel2angle(K, p):
    """Convert pixel coordinates to azimuth and elevation angles using camera matrix `K` and pixel `p`."""
    p = addcol0(p - K[2, 0:2])
...
def pixel2uvec(K, p):
    """Convert pixel coordinates `p` to unit vectors using camera matrix `K`."""
    p = addcol0(p - K[2, 0:2])
    p[:, 2] = K[0, 0]  # focal length (pixels)
    return uvec(p)
...
def fcnsigmarejection(x, srl=3.0, ni=3):
    """Apply sigma rejection to `x` with sigma level `srl` and iterations `ni`, returning filtered values and mask."""
...
def pscale(p3):  # normalizes camera coordinates so last column = 1
    """Normalize camera coordinates to make last column 1; input p3 is Nx3, returns Nx2 array."""
    return p3[:, 0:2] / p3[:, 2:3]


def worldPointsLicensePlate(country="EU"):  # Returns x, y coordinates of license plate
    """Return x, y coordinates of a standard license plate for given country; defaults to EU.
...
def cam2ned():  # x_ned(3x5) = R * x_cam(3x5)   - EQUALS -   x_ned(5x3) = x_cam(5x3) * R'
    """Convert camera coordinates to NED (North-East-Down) coordinate system."""
